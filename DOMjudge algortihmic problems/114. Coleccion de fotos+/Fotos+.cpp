...
bool resuelveCaso() {
	...
	for (int i = 0; i < nfotos; ++i) {
		ordenTitulo[i] = new tFoto;
		ordenTema[i] = ordenTitulo[i];
		getline(std::cin, ordenTitulo[i]->titulo);
	}
	for (int i = 0; i < nfotos; ++i) {
		getline(std::cin, ordenTitulo[i]->tema);

		tMatriz matriz(50, std::vector<int>(50, i));
		ordenTitulo[i]->matriz = matriz;
	}
	...
	std::cout << '\n';
	for (int i = 0; i < nfotos; ++i) {
		std::cout << ordenTema[i]->tema << " - " << ordenTema[i]->titulo << '\n';
		delete ordenTema[i];
	}
