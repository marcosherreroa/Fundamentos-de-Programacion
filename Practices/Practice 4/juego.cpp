...
bool ejecutarSecuenciaModo1(tJuego & juego, bool & joya) {//Comprueba los movimientos conforme se introducen
	...
	while (tecla != ENTER) {
		if (tecla != ABAJO && tecla != OTRA) {
			if (juego.jugadores[juego.turno].mano[tecla] > 0) {
				--juego.jugadores[juego.turno].mano[tecla];
				vcartas.push_back(tCarta(tecla));
				std::cout << SIMBOLOS[tecla];
			}
...
bool ejecutarSecuenciaModo2(tJuego & juego, bool & joya) { // Comprueba todas al final
	...
	std::vector<int> cartasSecuencia(NUMMOVIMIENTOS);
	tTecla tecla = leerTecla();
	while (tecla != ENTER && tecla != ABAJO) {
		if (tecla != OTRA) {
			vcartas.push_back(tCarta(tecla));
			++cartasSecuencia[tecla];
			std::cout << SIMBOLOS[tecla];
		}
		tecla = leerTecla();
	}
	...
	if (tecla == ABAJO) {
	...
	else if (cartasSecuencia[AVANZAR] > juego.jugadores[juego.turno].mano[AVANZAR] ||                     // se comprueba, antes de hacer nada,
		cartasSecuencia[GIROIZQUIERDA] > juego.jugadores[juego.turno].mano[GIROIZQUIERDA] ||              // si se dispone de las cartas necesarias
		cartasSecuencia[GIRODERECHA] > juego.jugadores[juego.turno].mano[GIRODERECHA] ||
		cartasSecuencia[LASER] > juego.jugadores[juego.turno].mano[LASER]) {
		...
		for (int i = 0; i < NUMMOVIMIENTOS; ++i) {
			juego.jugadores[juego.turno].mano[i] -= cartasSecuencia[i];
		}
...
bool jugar(tPuntuaciones & puntuaciones) {
	...
	for (int i = 0; i < juego.numJugadores; ++i) {
		liberarMazo(juego.jugadores[i].mazo);
